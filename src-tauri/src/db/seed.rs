...
/// Loescht alle Stammdaten in FK-sicherer Reihenfolge.
...
/// Erstellt einen Lehrer und gibt die ID zurueck.
...
/// Erstellt ein Fach und gibt die ID zurueck.
...
/// Erstellt eine Klasse und gibt die ID zurueck.
...
/// Erstellt einen Raum und gibt die ID zurueck.
...
/// Beispieldaten fuer ein grosses Gymnasium (Klasse 5-12).
fn seed_gymnasium(conn: &Connection) -> Result<SeedResult, AppError> {
    ...
    let fr = insert_subject(conn, "Franzoesisch", "Fr", "standard", 3)?;
    ...
    let teacher_data: TeacherSeedData = vec![
        ("Dr. Thomas Mueller", Some("mueller@gymnasium.de"), 0.85, 0.90, 1.0, 7, vec![ma, ph]),
        ...
        ("Hans-Juergen Wolf", Some("wolf@gymnasium.de"), 0.70, 0.75, 1.0, 7, vec![ge, gk]),
        ("Martina Klein", Some("klein@gymnasium.de"), 0.85, 0.85, 0.75, 6, vec![mu]),
        ("Joerg Schroeder", Some("schroeder@gymnasium.de"), 0.80, 0.70, 1.0, 7, vec![ma, inf]),
        ...
        ("Birgit Krueger", Some("krueger@gymnasium.de"), 0.80, 0.75, 1.0, 7, vec![en, de]),
        ...
        ("Gerald Koenig", None, 0.90, 0.75, 1.0, 7, vec![ma]),
        ("Kathleen Otto", None, 0.75, 0.85, 1.0, 7, vec![en, de]),
        ("Ralf Grosse", None, 0.80, 0.70, 1.0, 7, vec![ph, ma]),
        ...
        ("Ines Boehm", None, 0.80, 0.80, 0.5, 4, vec![gk]),
...
/// Beispieldaten fuer eine mittelgrosse Grundschule (Klasse 1-4).
fn seed_grundschule(conn: &Connection) -> Result<SeedResult, AppError> {
    ...
    // --- Lehrkraefte (25) ---
    let teacher_data: TeacherSeedData = vec![
        ("Katrin Mueller", Some("k.mueller@grundschule.de"), 0.85, 0.90, 1.0, 6, vec![de, ma, su]),
        ...
        ("Doris Krueger", None, 0.85, 0.75, 1.0, 6, vec![ma, de, su]),
...
/// Beispieldaten fuer eine kleine Mittelschule/Oberschule (Klasse 5-10).
fn seed_mittelschule(conn: &Connection) -> Result<SeedResult, AppError> {
    ...
    // --- Lehrkraefte (35) ---
    let teacher_data: TeacherSeedData = vec![
        ("Andreas Mueller", Some("a.mueller@mittelschule.de"), 0.80, 0.85, 1.0, 7, vec![ma, ph]),
        ...
        ("Ralph Krueger", Some("r.krueger@mittelschule.de"), 0.80, 0.80, 0.75, 6, vec![ch]),
...
/// Hauptfunktion: Loescht alle Daten und erstellt Beispieldaten fuer den gewaehlten Schultyp.
